...
class ReaderWidgetState extends ConsumerState<Reader>
  ...
  // PDF and MOBI state classes are private to their files, so we use the
  // generic `State<StatefulWidget>` key type and rely on `is
  // DestinationCapable` for dispatch.
  final GlobalKey<State<StatefulWidget>> _pdfKey = GlobalKey();
  final GlobalKey<State<StatefulWidget>> _mobiReaderKey = GlobalKey();
  ...
  void saveEpubProgress(double progressFraction) {
    progress.saveEpubProgress(
        bookId: widget.id, progress: progressFraction);
  }
  ...
  void _showSettings() {
    ...
    if (widget.type == 'epub') {
      ...
      if (epubState != null) {
        ...
        if (readerState != null) {
          currentScrollMode =
              readerState.readingMode == ReadingMode.page ? 'page' : 'scroll';
