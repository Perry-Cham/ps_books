...
class CustomChatWidget extends StatefulWidget {
  ...
  /// Optional custom context menu builder for message bubbles.
  final Widget Function(String messageText)? customContextMenu;
...
class _CustomChatWidgetState extends State<CustomChatWidget> {
                      ...
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            // Custom context menu (three-dot icon)
                                    ...
                                    showMenu<String>(
                                      ...
                                      items: [
                                        PopupMenuItem<String>(
                                          enabled: false,
                                          child: widget.customContextMenu!(
                                              message.text),
                                        ),
                                      ],
                                    );
  ...
  Widget _buildMessageContent(ChatMessage message, BuildContext context) {
    ...
    // Auto-treat AI messages as markdown when streaming is enabled,
    // so users don't need to set isMarkdown: true on every AI message.
    final isAiMessage = message.user.id != widget.currentUser.id;
    final shouldRenderAsMarkdown =
        message.isMarkdown || (isAiMessage && widget.streamingEnabled);
    ...
    if (shouldRenderAsMarkdown) {
      ...
      if (needsInteractiveMarkdown) {
        ...
        // Math rendering takes priority over streaming animation.
        if (shouldAnimate && widget.streamingEnabled && !widget.enableMathRendering) {
          ...
          // Takes priority over streaming when both are enabled.
        ...
        } else if (shouldAnimate && widget.streamingEnabled) {
          textWidget = StreamingText(
            text: message.text,
            style: textStyle,
            typingSpeed: widget.streamingTypingSpeed,
    ...
    // Wrap in SelectionArea so users can long-press to select and copy text.
    // Skip SelectionArea during streaming animation since animated text
    // isn't practically selectable.
    final selectableTextWidget = shouldAnimate
        ? directionalTextWidget
        : SelectionArea(child: directionalTextWidget);
