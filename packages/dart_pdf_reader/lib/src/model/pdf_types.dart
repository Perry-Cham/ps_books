...
import '../utils/pdf_doc_encoding.dart';
...
class PDFLiteralString extends PDFStringLike {
  ...
  ///
  /// Follows the same algorithm as pdfjs's `stringToPDFString`:
  ///   1. UTF-16BE BOM (FE FF) -> decode as UTF-16BE
  ...
  ///      undefined slots), stripping PDF language escape sequences.
  ///
  /// This fixes the previous bug where non-ASCII PDFDocEncoding strings (e.g.
  /// the document Title or Author) were decoded as UTF-8 with `allowMalformed`,
  /// producing U+FFFD replacements (seen as "unknown") or swallowing following
  /// ASCII bytes (seen as a "truncated filename").
  @override
  String asString() {
    return decodePdfTextString(_value);
  }
