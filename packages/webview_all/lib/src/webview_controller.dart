...
class WebViewController {
  ...
  /// Request keyboard focus for the webview.
  ///
  /// On Linux this calls `gtk_widget_grab_focus()` on the native WebKit
  /// widget, routing all keyboard input to the webview.
  /// On other platforms this is a no-op.
  ///
  /// Typically used when [focusable] is `false` (see [LinuxWebViewController])
  /// and you temporarily need to type inside the webview.
  Future<void> requestFocus() async {
    try {
      return await (platform as dynamic).requestFocus();
    } on NoSuchMethodError {
      // Not supported on this platform.
    }
  }

  /// Release keyboard focus from the webview, returning it to Flutter.
  ///
  /// On Linux this calls `gtk_window_set_focus(toplevel, NULL)`,
  /// which removes GTK focus from the webview and lets Flutter's
  /// IM context receive keyboard events again.
  /// On other platforms this is a no-op.
  ///
  /// Typically used in a split-view layout: wrap Flutter interactive
  /// widgets with a `Listener` whose `onPointerDown` calls this method.
  Future<void> releaseFocus() async {
    try {
      return await (platform as dynamic).releaseFocus();
    } on NoSuchMethodError {
      // Not supported on this platform.
    }
  }
  ...
  ///
  /// On Linux this terminates the WebKit web process (~200 MB per instance)
  /// and destroys the native widget. On Web this removes the iframe from the
  /// DOM. Call this when navigating away from the WebView to prevent
  /// zombie WebKit processes from accumulating.
  Future<void> dispose() async {
    try {
      return await (platform as dynamic).dispose();
    } on NoSuchMethodError {
      // Not supported on this platform.
    }
  }
