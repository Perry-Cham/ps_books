...
void instance_method_call_cb(FlMethodChannel* channel,
  ...
  if (strcmp(method, "setFrame") == 0) {
    ...
    if (webview->visible) {
      ...
      // Focus is no longer auto-grabbed here.  The Dart layer
      // explicitly calls requestFocus() when appropriate
      // (controlled by the controller's `focusable` flag).
    } else {
      gtk_widget_hide(widget);
      // GTK automatically moves focus to the next widget in the
      // focus chain (the Flutter view) when the focused widget is
      // hidden, so keyboard input returns to Flutter.
    }
  ...
  if (strcmp(method, "requestFocus") == 0) {
    gtk_widget_grab_focus(GTK_WIDGET(webview->web_view));
    respond(method_call, success_response());
    return;
  }

  if (strcmp(method, "releaseFocus") == 0) {
    GtkWidget* toplevel =
  ...
  if (strcmp(method, "dispose") == 0) {
    // Release GTK focus back to Flutter before destroying.
    GtkWidget* toplevel =
        gtk_widget_get_toplevel(GTK_WIDGET(webview->web_view));
    if (GTK_IS_WINDOW(toplevel)) {
      gtk_window_set_focus(GTK_WINDOW(toplevel), NULL);
    ...
    webkit_web_view_terminate_web_process(webview->web_view);

    // Remove from the plugin's webview hash table, which triggers
    // destroy_linux_webview() -> gtk_widget_destroy + g_object_unref.
    g_hash_table_remove(webview->plugin->webviews,
                        GINT_TO_POINTER(webview->id));
