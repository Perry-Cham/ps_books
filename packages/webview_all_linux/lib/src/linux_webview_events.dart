      ...
      case 'httpError':
        if (_navigationDelegate != null) {
          ...
          _navigationDelegate!.handleHttpError(
            HttpResponseError(
              request: LinuxWebResourceRequest(
                uri: uri ?? Uri(),
                method: '${event['method'] ?? 'GET'}',
                headers: (event['requestHeaders'] as Map<Object?, Object?>?)
                        ?.map(
                          (Object? k, Object? v) =>
                              MapEntry<String, String>('$k', '$v'),
                        ) ??
                    const <String, String>{},
                isForMainFrame: event['isForMainFrame'] != false,
              ),
              response: LinuxWebResourceResponse(
                uri: uri,
                statusCode: (event['statusCode'] as num?)?.toInt() ?? 0,
                mimeType: '${event['mimeType'] ?? ''}',
                headers: (event['headers'] as Map<Object?, Object?>?)
                        ?.map(
                          (Object? k, Object? v) =>
                              MapEntry<String, String>('$k', '$v'),
                        ) ??
