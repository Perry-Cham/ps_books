...
class LinuxWebViewController extends PlatformWebViewController {
  LinuxWebViewController(PlatformWebViewControllerCreationParams params)
    : _linuxParams = params is LinuxWebViewControllerCreationParams
          ? params
          : null,
  ...
  final LinuxWebViewControllerCreationParams? _linuxParams;
  ...
  Future<void> _applyInitialSettings() async {
    final LinuxWebViewControllerCreationParams? p = _linuxParams;
    if (p == null) return;
    final Map<String, Object?> settings = <String, Object?>{};
    if (p.developerExtrasEnabled != null) {
      settings['developerExtrasEnabled'] = p.developerExtrasEnabled;
    }
    if (p.javascriptCanOpenWindowsAutomatically != null) {
      settings['javascriptCanOpenWindowsAutomatically'] =
          p.javascriptCanOpenWindowsAutomatically;
    }
    if (p.mediaPlaybackRequiresUserGesture != null) {
      settings['mediaPlaybackRequiresUserGesture'] =
          p.mediaPlaybackRequiresUserGesture;
    }
    if (p.mediaPlaybackAllowsInline != null) {
      settings['mediaPlaybackAllowsInline'] = p.mediaPlaybackAllowsInline;
    }
    if (p.pageCacheEnabled != null) {
      settings['pageCacheEnabled'] = p.pageCacheEnabled;
    }
    if (p.allowFileAccessFromFileUrls != null) {
      settings['allowFileAccessFromFileUrls'] = p.allowFileAccessFromFileUrls;
    }
    if (p.allowUniversalAccessFromFileUrls != null) {
      settings['allowUniversalAccessFromFileUrls'] =
          p.allowUniversalAccessFromFileUrls;
    }
    if (p.zoomTextOnly != null) {
      settings['zoomTextOnly'] = p.zoomTextOnly;
    }
    if (p.defaultFontSize != null) {
      settings['defaultFontSize'] = p.defaultFontSize;
    }
    if (p.defaultMonospaceFontSize != null) {
      settings['defaultMonospaceFontSize'] = p.defaultMonospaceFontSize;
    }
    if (p.minimumFontSize != null) {
      settings['minimumFontSize'] = p.minimumFontSize;
    }
    if (p.zoomFactor != null) {
      settings['zoomFactor'] = p.zoomFactor;
    }
    if (settings.isNotEmpty) {
      await _invoke<void>('applySettings', settings);
    }
  }
  ...
  Future<void> openDevTools() {
    return _invoke<void>('openDevTools');
  }
  ...
  Future<void> setMediaPlaybackRequiresUserGesture(bool required) {
    return _invoke<void>(
  ...
  Future<void> setMediaPlaybackAllowsInline(bool allowsInline) {
    return _invoke<void>(
      'setMediaPlaybackAllowsInline',
      <String, Object?>{'allowsInline': allowsInline},
    );
  }
  ...
  Future<void> setAllowFileAccessFromFileUrls(bool allowed) {
    return _invoke<void>(
  ...
  Future<void> setAllowUniversalAccessFromFileUrls(bool allowed) {
    return _invoke<void>(
  ...
  Future<void> setZoomTextOnly(bool zoomTextOnly) {
  ...
  /// Whether the webview should automatically grab GTK keyboard focus
  /// when it first becomes visible.
  ///
  /// When [focusable] is [true] (default), the webview auto-grabs keyboard
  /// focus each time it transitions from hidden to visible. This is the
  /// existing behaviour and is correct for full-screen readers.
  ///
  /// When [focusable] is [false], the webview never auto-grabs focus.
  /// Keyboard input stays with Flutter, which is the correct behaviour for
  /// split-view layouts where a Flutter text field (e.g. AI chat) needs
  /// keyboard input alongside the webview.
  ///
  /// Regardless of this flag, you can always call [requestFocus] and
  /// [releaseFocus] for explicit manual control.
  bool focusable = true;

  /// Explicitly grab GTK keyboard focus, routing all keyboard input to
  /// the webview.
  ///
  /// Useful when [focusable] is [false] and you temporarily need to
  /// type inside the webview (e.g. fill a web form).
  Future<void> requestFocus() {
    return _invoke<void>('requestFocus');
  }

  /// Release GTK keyboard focus from the webview, returning keyboard
  /// input to Flutter's rendering surface.
  ///
  /// Call this when a Flutter widget needs keyboard input while the
  /// webview is still visible (e.g. in a split-view layout).
  Future<void> releaseFocus() {
    return _invoke<void>('releaseFocus');
  }
