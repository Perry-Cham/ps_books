...
/// * Parse various comic book archive formats (CBZ, CBT, ZIP, TAR, CBR, RAR)
...
class ComicReaderParser {
  ...
  Future<ComicModel> pickAndParseComicFile(String filePath) async {
    try {
      ...
      final metadata = await _parseComicXml(extractedFiles);
      return ComicModel(
        comicPages: extractedFiles,
  ...
  /// Parses comic file bytes and returns a [ComicModel] with extracted pages.
  ///
  /// This method:
  /// * Determines the file type from the provided [fileName]
  /// * Extracts the comic name from the filename
  /// * Decodes the archive bytes based on file extension
  /// * Returns a [ComicModel] with sorted page paths and comic name
  ///
  /// Parameters:
  /// * [bytes] - The raw bytes of the comic archive file
  /// * [fileName] - The name of the file (used for extension detection and naming)
  ///
  /// Returns:
  /// * A [ComicModel] containing the list of extracted image paths and comic name
  ///
  ...
  /// * [Exception] for general parsing errors
  /// * [UnsupportedError] for unsupported file formats
  Future<ComicModel> parse(Uint8List bytes, String fileName) async {
    try {
      if (bytes.isEmpty) {
      ...
      currentComicName = path.basenameWithoutExtension(fileName);
      
      List<String> extractedFiles;

      if (fileExt == '.cbr' || fileExt == '.rar') {
        final tempDir = await getTemporaryDirectory();
        final rarFile = File('${tempDir.path}/$currentComicName.rar');
        await rarFile.writeAsBytes(bytes);
        try {
          final extractor = UnrarExtractor();
          final archiveDirName = '${currentComicName}_rar';
          final comicDir = Directory('${tempDir.path}/$archiveDirName');
          if (await comicDir.exists()) {
            await comicDir.delete(recursive: true);
          }
          await comicDir.create(recursive: true);
          extractor.extractAll(rarFile.path, comicDir.path);
          extractedFiles = await _collectImageFilesFromDir(comicDir.path);
        } finally {
          if (await rarFile.exists()) {
            await rarFile.delete();
          }
        }
      } else {
        ArchiveType type;
        ...
        extractedFiles = await _extractImageFiles(archive, typeStr);
      }
      ...
      throw Exception('Error parsing comic bytes: $e');
    }
  }
  ...
  Future<List<String>> _parseArchiveByExtension(File file, String fileExt) async {
    switch (fileExt) {
      ...
      case '.cbr':
        return await _extractRarArchive(file, '.cbr');
      case '.rar':
        return await _extractRarArchive(file, '.rar');
      default:
        throw UnsupportedError('Unsupported comic format: $fileExt');
    }
  }
  ...
  Future<List<String>> _extractImageFiles(Archive archive, String archiveType) async {
    try {
      ...
      // Extract image files and ComicInfo.xml
      for (final file in archive) {
        if (!file.isFile) continue;
        
        final fileName = file.name.toLowerCase();
        final isImageFile = validExtensions.any((ext) => fileName.endsWith(ext));
        final isComicInfo = path.basename(fileName) == 'comicinfo.xml';
        
        if (isImageFile || isComicInfo) {
          try {
  ...
  /// Extracts a RAR archive file to a temporary directory and returns image file paths.
  ///
  /// Uses the [UnrarExtractor] to extract files from RAR/CBR archives.
  ///
  /// Parameters:
  /// * [file] - The RAR/CBR file to extract
  /// * [fileExt] - The original file extension (e.g., '.cbr')
  ///
  /// Returns:
  /// * A list of paths to the extracted image files
  Future<List<String>> _extractRarArchive(File file, String fileExt) async {
    try {
      final extractor = UnrarExtractor();
      final tempDir = await getTemporaryDirectory();
      final archiveDirName = '${currentComicName}_rar';
      ...
      extractor.extractAll(file.path, comicDir.path);

      return await _collectImageFilesFromDir(comicDir.path);
    } catch (e) {
      print('Error extracting RAR archive: $e');
      throw Exception('Failed to parse RAR archive: $e');
    }
  }

  /// Collects image file paths from a directory by scanning for valid image extensions.
  ///
  /// Parameters:
  ...
  Future<List<String>> _collectImageFilesFromDir(String dirPath) async {
    final validExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp'];
    ...
    if (extractedFiles.isEmpty) {
      print('Warning: No image files found in extracted RAR archive');
    }

    return extractedFiles;
  }

  /// Parses metadata from ComicInfo.xml if it exists in the extracted files directory.
  ///
  /// Parameters:
  /// * [extractedFiles] - List of paths to extracted image files.
  ///
  /// Returns:
  /// * A [ComicMetadata] object if ComicInfo.xml exists and is parsed, otherwise null.
  Future<ComicMetadata?> _parseComicXml(List<String> extractedFiles) async {
    try {
      if (extractedFiles.isEmpty) return null;

      final directory = path.dirname(extractedFiles.first);
      final xmlFile = File(path.join(directory, 'ComicInfo.xml'));

      if (!await xmlFile.exists()) return null;

      final content = await xmlFile.readAsString();
      final document = XmlDocument.parse(content);
      final comicInfo = document.findElements('ComicInfo').firstOrNull;

      if (comicInfo == null) return null;

      String? getValue(String tag) {
        return comicInfo.findElements(tag).firstOrNull?.innerText.trim();
      }
      ...
      print('Error parsing ComicInfo.xml: $e');
      return null;
    }
  }
}
