...
class BookCoverReader {
  static Future<images.Image?> readBookCover(EpubBookRef bookRef) async {
    ...
    // EPUB 3 strategy: look for properties="cover-image" on manifest items
    final manifestItems = bookRef.schema?.package?.manifest?.items;
    if (manifestItems != null && manifestItems.isNotEmpty) {
      final coverManifestItem = manifestItems.firstWhereOrNull((item) =>
          item.properties?.toLowerCase().contains('cover-image') == true);
      if (coverManifestItem != null &&
          bookRef.content?.images.containsKey(coverManifestItem.href) ==
              true) {
        var coverImageContentFileRef =
