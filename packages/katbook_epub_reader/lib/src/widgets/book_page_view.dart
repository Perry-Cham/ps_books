...
class BookPageViewState extends State<BookPageView>
  ...
  // Handle Keyboard inputs in page view
  ...
  List<List<_PageSegment>> _buildPagesSync(Size pageSize) {
    ...
    for (int i = 0; i < widget.paragraphs.length; i++) {
      ...
      if (para.isChapterStart && para.chapterTitle != null) {
        final titleHeight = _measureChapterTitleHeight(
          para.chapterTitle!,
          contentWidth,
        );
      ...
      while (remainingText.isNotEmpty) {
        ...
        double dropCapHeight = needsDropCap && isFirstSegment
            ? widget.fontSize * 3.5
            : 0;
  ...
  Widget build(BuildContext context) {
    ...
    final pageHeight = screenSize.height * 0.90;
  ...
  Color _getPageColor() {
    if (widget.themeData.isDark) {
      return Color.lerp(
        widget.themeData.backgroundColor,
        Colors.grey[850],
        0.3,
      )!;
    } else {
        ...
        boxShadow: [
          BoxShadow(
            color: Colors.black.withValues(
              alpha: widget.themeData.isDark ? 0.5 : 0.2,
            ),
            blurRadius: 15,
          ...
          BoxShadow(
            color: Colors.black.withValues(
              alpha: widget.themeData.isDark ? 0.3 : 0.1,
            ),
            blurRadius: 3,
            ...
            Positioned(
              ...
              child: Center(
                child: Text(
                  '— ${pageIndex + 1} —',
                  style: GoogleFonts.literata(
                    color: widget.themeData.secondaryTextColor.withValues(
                      alpha: 0.6,
                    ),
                    fontSize: 11,
                  ),
    ...
    for (final node in element.nodes) {
      if (node is dom.Text) {
        ...
        switch (tag) {
          case 'strong':
          case 'b':
            spanStyle = (spanStyle ?? const TextStyle()).copyWith(
              fontWeight: FontWeight.bold,
            );
            break;
          case 'em':
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: Text(firstLetter, style: _getDropCapStyle()),
          ),
          Expanded(
            child: Text(
              restOfText,
