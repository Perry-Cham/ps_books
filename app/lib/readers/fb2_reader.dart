...
class FB2ReaderState extends State<FB2Reader> implements DestinationCapable {
  ...
  void _collectBinaries(XmlDocument document) {
    ...
    for (var bin in binaryElements) {
      ...
      if (id.isNotEmpty && base64Data.isNotEmpty) {
        binaries.add(
          FB2Binary(id: id, contentType: contentType, base64Data: base64Data),
        );
      }
    }
  }
  ...
  Widget _processEpigraph(XmlElement epigraphEl) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border(
          left: BorderSide(color: Colors.grey.withValues(alpha: 0.5), width: 3),
        ),
      ),
      child: Column(
      ...
      if (child.localName == 'stanza') {
        ...
        stanzas.add(
          Padding(
            ...
            child: Column(children: verses),
          ),
        );
      }
    }

    return Column(children: stanzas);
  }
  ...
  void _flatten() {
    ...
    for (var chapter in chapters) {
      ...
      final chapterIdx = chapter.content.isNotEmpty
          ? chapter.content.first.chapterIndex
          : -1;

      for (var titleWidget in chapter.title) {
        renderable.add(titleWidget);
        indexedItems.add(
          IndexedParagraph(chapterIndex: chapterIdx, content: titleWidget),
        );
      }
  ...
  void setTheme(ReaderTheme theme) {
    setState(() {
      isDarkMode = theme == ReaderTheme.dark;
    });
  }
